...
@click.option("--share-probility", "-p", "share", default="clone", prompt=True, type=click.Choice(['clone', 'uniform', 'first'])) #get default from environment
...
def tag_wsd(input_file, input_type, output_file, api_key, share, combine):
    files = ls(input_file)
    if not files:
    ...
    for f in files:
        if not os.path.isfile(out(f)):
            data = tag_document(_json(f), api_key, share, combine, data_type=input_type)
